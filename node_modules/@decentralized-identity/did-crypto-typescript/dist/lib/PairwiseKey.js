...
class PairwiseKey {
    ...
    async generateHashForPrime(crypto, _inx, key, data) {
        ...
        const deterministicNumber = new DidKey_1.default(crypto, alg, key, true);
    ...
    async executeRounds(crypto, rounds, inx, key, data) {
        const signature = await rounds[inx](crypto, inx, key, data);
     ...
     * @param exportable True if the key is exportable.
     */
    async generateRsaPairwiseKey(didMasterKey, crypto, algorithm, keyUse, exportable) {
        // Set the ke\y size
        const keySize = algorithm.modulusLength || 1024;
        // Get peer id
        const peerId = Buffer.from(this._peerId);
        // Get deterministic base number for p
        const pBase = await this.generateDeterministicNumberForPrime(crypto, keySize / 2, didMasterKey, peerId);
        // Get deterministic base number for q
        const qBase = await this.generateDeterministicNumberForPrime(crypto, keySize / 2, pBase, peerId);
        ...
        return new DidKey_1.default(crypto, algorithm, jwk, exportable);
    }
    /**
    ...
    getPrime(primeBase) {
        ...
        const prime = this.generatePrime(qArray);
     ...
     * @param exportable True if the key is exportable.
     */
    async generateEcPairwiseKey(didMasterKey, crypto, 
    // tslint:disable-next-line:completed-docs
    algorithm, exportable) {
        // Generate peer key
        const alg = { name: 'hmac', hash: 'SHA-256' };
        const hashDidKey = new DidKey_1.default(crypto, alg, didMasterKey, true);
        const signature = await hashDidKey.sign(Buffer.from(this._peerId));
